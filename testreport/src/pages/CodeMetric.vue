        ...
        <div class="md-layout-item md-xlarge-size-100 md-large-size-100 md-medium-size-75 md-small-size-50 md-xsmall-size-100">
            <md-table md-card>
                ...
                <template v-for="(item,key) in condFunct">
                    <md-table-row :key="key">
                        <md-table-cell>{{item.name}}</md-table-cell>
                        <md-table-cell><span :style="'background-color:'+(item.PTH?(item.PTH.max>80?'#33BCFF':(item.PTH.max>80?'#CCEEFF':'')):'')" >{{item.PTH?(item.PTH.min===item.PTH.max?item.PTH.min:item.PTH.min+'...'+item.PTH.max):'n/a'}}</span></md-table-cell>
                        <md-table-cell><span :style="'background-color:'+(item.CYC?(item.CYC.max>20?'#33BCFF':(item.CYC.max>10?'#CCEEFF':'')):'')" >{{item.CYC?(item.CYC.min===item.CYC.max?item.CYC.min:item.CYC.min+'...'+item.CYC.max):'n/a'}}</span></md-table-cell>
                        <md-table-cell><span :style="'background-color:'+(item.CAL?(item.CAL.max>10000000?'#33BCFF':(item.CAL.max>7?'#CCEEFF':'')):'')" >{{item.CAL?(item.CAL.min===item.CAL.max?item.CAL.min:item.CAL.min+'...'+item.CAL.max):'n/a'}}</span></md-table-cell>
                        <md-table-cell><span :style="'background-color:'+(item.PAR?(item.PAR.max>10000000?'#33BCFF':(item.PAR.max>5?'#CCEEFF':'')):'')" >{{item.PAR?(item.PAR.min===item.PAR.max?item.PAR.min:item.PAR.min+'...'+item.PAR.max):'n/a'}}</span></md-table-cell>
                        <md-table-cell><span :style="'background-color:'+(item.MIF?(item.MIF.max>6?'#33BCFF':(item.MIF.max>4?'#CCEEFF':'')):'')" >{{item.MIF?(item.MIF.min===item.MIF.max?item.MIF.min:item.MIF.min+'...'+item.MIF.max):'n/a'}}</span></md-table-cell>
                        <md-table-cell><span :style="'background-color:'+(item.BAK?(item.BAK.max>100?'#33BCFF':(item.BAK.max>0?'#CCEEFF':'')):'')" >{{item.BAK?(item.BAK.min===item.BAK.max?item.BAK.min:item.BAK.min+'...'+item.BAK.max):'n/a'}}</span></md-table-cell>

                        <md-table-cell><span :style="'background-color:'+(item.GTO?(item.GTO.max>0?'#33BCFF':(item.GTO.max>0?'#CCEEFF':'')):'')" >{{item.GTO?(item.GTO.min===item.GTO.max?item.GTO.min:item.GTO.min+'...'+item.GTO.max):'n/a'}}</span></md-table-cell>
                        <md-table-cell><span :style="'background-color:'+(item.M19?(item.M19.max>10?'#33BCFF':(item.M19.max>1?'#CCEEFF':'')):'')" >{{item.M19?(item.M19.min===item.M19.max?item.M19.min:item.M19.min+'...'+item.M19.max):'n/a'}}</span></md-table-cell>
...
export default {
  ...
  methods:{
    getResultCompliance(){
        ...
        console.log(' this.TestRuns_QACSummary', this.TestRuns_QACSummary)
